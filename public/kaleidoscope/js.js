...
function createImg(){
  ...
  _ctx_R.scale(-1, 1);
  _ctx_R.drawImage(base, -img_R.width, 0, base.width*scale, base.height*scale);
}
...
function draw(alternateMode){
  ...
  ctx.translate((-i+(i%3==0?0.5:i%3==1?1.5:-0.5))*pattern_W, -pattern_H+offset);
  ctx.translate(0, -offset);
  ctx.rotate(120*Math.PI/180);
  ctx.translate(0, offset);
  var j=0;
...
function tile(){
  var rowData = ctx.getImageData(0,0,pattern_W*3,Math.floor(pattern_H*2));
  for(var i=0; Math.floor(pattern_H*2)*i<c.height+Math.sqrt(3)/2*pattern_W; i++){
